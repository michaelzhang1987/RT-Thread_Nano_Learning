...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  if(huart == &huart5)
  {
    HAL_UART_Receive_IT(&huart5,&UartBuffer[3],1);
    rt_mq_send(uart5_mq,&UartBuffer[3],1);
  }
  else if(huart == &huart4)
  {
    HAL_UART_Receive_IT(&huart4,&UartBuffer[4],1);
...
{
  uart5_mq = rt_mq_create("uart5_mq",1,200,RT_IPC_FLAG_FIFO);
  uart4_mq = rt_mq_create("uart4_mq",1,200,RT_IPC_FLAG_FIFO);
  uart2_mq = rt_mq_create("uart2_mq",1,200,RT_IPC_FLAG_FIFO);
  return 0;
}
